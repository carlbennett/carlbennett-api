...
class VersionInfo
{
    ...
    public static function get(): array
    {
        ...
        $r['api'] = self::deploymentVersion() ?? self::reflectVersion();
        $r['asset'] = self::assetVersion($r['api']);
        $r['php'] = \phpversion();
    ...
    {
        return (
            !(\CarlBennett\API\Libraries\Core\Config::instance()->root['router']['asset_versioning'] ?? false) ? '' :
    ...
    /**
     * Determines if the client's user agent is a web browser or a different type of client.
     * 
     * @param string $user_agent The user agent to check patterns against.
     *                           The default empty string will use \getenv('HTTP_USER_AGENT') instead.
     * @return bool True if the client is a web browser, false otherwise.
     */
    public static function isBrowser(string $user_agent = ''): bool
    {
        $ua = !empty($user_agent) ? $user_agent : \getenv('HTTP_USER_AGENT');
        $names = [
            // Browser names
            'Mozilla',
            'Firefox',
        ...
        {
            if (\stripos($ua, $name) !== false) return true;
        }
    ...
    private static function reflectVersion(): ?array
    {
        ...
        if (\file_exists($git_dir) && \is_dir($git_dir))
        {
            ...
            if (!empty($identifier)) $values[0] = \trim($identifier);
            if (!empty($hash)) $values[1] = \trim($hash);
            if (!empty($timestamp)) $values[2] = \trim($timestamp);
            if (!empty($license)) $values[3] = \trim($license);
        }
